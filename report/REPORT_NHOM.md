...
**Nhóm:** IMBACK
**Thành viên:** Nguyễn Tuấn Vũ, Phan Huy Hoàng, Lương Sỹ Linh
**Ngày:** 03/08/2026
...
**Thành viên 3 — Lương Sỹ Linh**

- **Loại chiến lược:** Ablation-driven retrieval + evidence grounding trên `RecursiveChunker(chunk_size=700)` và local multilingual embedding.
- **Mô tả & lý do chọn:** Lương thử SentenceChunker và FixedSizeChunker trước, sau đó so sánh không chỉ số lượng chunk mà cả khả năng đưa đúng bằng chứng vào top-3. Ở tầng đánh giá, mỗi kết quả còn được kiểm tra bằng `doc_id` và các cụm bằng chứng bắt buộc; score cosine cao nhưng thiếu điều khoản không được xem là grounded.
- **Code/evaluation snippet:**
...
evidence_match = (
    ...
    and contains_evidence(result["content"], evidence_terms)
)
```
...
| Lương Sỹ Linh  | Ablation + RecursiveChunker 700 + evidence grounding | 10/10                | Phân biệt được đúng tài liệu với đúng đoạn bằng chứng; ghi nhận rõ rank và evidence match | Sentence/Fixed-size baseline bị nhiễu bởi tài liệu OCR; tầng kiểm chứng cần thêm cụm bằng chứng |

**Chiến lược nào tốt nhất cho chủ đề này? Tại sao?**

> Kết luận của nhóm là dùng pipeline kết hợp: `RecursiveChunker` giữ cấu trúc điều khoản, local embedding tìm ứng viên theo ngữ nghĩa, metadata filter giảm nhiễu theo vai trò, và evidence check xác nhận câu trả lời có căn cứ. Recursive đơn thuần chưa đủ vì có thể trả về đúng tài liệu nhưng sai đoạn; ngược lại, evidence check không thể cứu một corpus đã bị cắt sai. Kết quả của ba thành viên cho thấy pipeline kết hợp đạt 5/5 câu có bằng chứng trong top-3, trong đó bằng chứng đứng top-1 ở các câu 1 và 3.
...
### So sánh kết quả của ba thành viên

| Thành viên     | Câu 1 | Câu 2 | Câu 3 | Câu 4 | Câu 5 | Tổng relevant top-3 |
| -------------- | ----: | ----: | ----: | ----: | ----: | ------------------: |
| Nguyễn Tuấn Vũ |    Có |    Có |    Có |    Có |    Có |                 5/5 |
| Phan Huy Hoàng |    Có |    Có |    Có |    Có |    Có |                 5/5 |
| Lương Sỹ Linh  |    Có |    Có |    Có |    Có |    Có |                 5/5 |

Ở mức thứ hạng, kết quả của Lương là rank `1, 3, 1, 2, 2`. Điều này cho thấy metric top-3 đạt tối đa nhưng chất lượng top-1 vẫn khác nhau giữa các câu; câu 2 cần filter người bán và vẫn cần chunking theo đề mục để đưa bằng chứng lên cao hơn.
...
> 4. Evidence check giúp nhóm phân biệt `relevant document` và `relevant chunk`, tránh đánh giá quá cao một kết quả chỉ vì score cosine cao.

**Bài học rút ra khi so sánh trong nhóm:**

> Cùng một bộ luật nhưng chiến lược chia nhỏ khác nhau tạo ra hiệu suất khác nhau rõ rệt. Fixed-size dễ tạo chunk có nội dung cắt ngang, trong khi SentenceChunker có thể gộp quá nhiều câu vào một chunk làm mất độ mạch lạc. RecursiveChunker là phương án cân bằng tốt nhất cho văn bản pháp lý vì nó giữ ranh giới theo heading/paragraph. Tuy nhiên, báo cáo của Lương cho thấy cần thêm bước kiểm chứng bằng chứng: một chunk cùng chủ đề chưa chắc chứa điều kiện hoặc ngoại lệ mà câu hỏi yêu cầu.

**Nếu làm lại, nhóm sẽ thay đổi gì trong chiến lược dữ liệu (data strategy)?**

> Nếu làm lại, nhóm sẽ chia theo từng đề mục rõ ràng hơn và gắn thêm `section_title`/`policy_clause` vào metadata để retrieval dễ đối chiếu với một điều khoản cụ thể. Nhóm cũng sẽ lưu chỉ mục cụm bằng chứng hoặc từ khóa hiếm để rerank sau semantic retrieval, đồng thời ghi nhận chất lượng OCR của từng tài liệu. Ngoài ra, nhóm sẽ ưu tiên `recursive` với `chunk_size` thấp hơn ở các phần có danh sách dài và dùng metadata filter mạnh hơn khi câu hỏi nhắm vào vai trò `buyer` hoặc `seller`.
